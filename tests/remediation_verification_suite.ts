...
import { AuditEvaluator } from '../backend/audit/evaluator.ts';
...
async function runRemediationVerificationSuite() {
  ...
  const spoofedEvidence = matcher.evaluateEvidence('f-spoof', 'GST_Registration_Certificate.pdf', '/tmp/GST_Registration_Certificate.pdf', gstParam, { text: 'This is just a lorem ipsum document with no GST details.', warnings: [] });
  ...
  assert.ok(spoofResult.warnings.some(w => w.includes('filename spoofing')), 'Warning about filename spoofing expected');
  console.log('  ✔ Filename spoofing correctly prevented from producing false PASS.');
  ...
  const missingExpiryEvidence = matcher.evaluateEvidence('f-noexp', 'Insurance_Policy.pdf', '/tmp/Insurance_Policy.pdf', insuranceParam, { text: 'Commercial General Liability Policy POL123456 active starting 2025-01-01.', warnings: [] });
  ...
  const findings = scanner.evaluateRules({ text: dummyText, metadata: {}, warnings: [] }, BUILTIN_RULES);
  ...
  const aadhaarFindings = scanner.evaluateRules({ text: aadhaarText, metadata: {}, warnings: [] }, BUILTIN_RULES);
  ...
  const ev1 = matcher.evaluateEvidence('f1', 'Doc1.pdf', '/tmp/Doc1.pdf', gstParam, { text: 'GSTIN 27AAAAA0000A1Z5 Issued to John Smith', warnings: [] })!;
  const ev2 = matcher.evaluateEvidence('f2', 'Doc2.pdf', '/tmp/Doc2.pdf', gstParam, { text: 'GST Certificate registered under Jane Doe', warnings: [] })!;
